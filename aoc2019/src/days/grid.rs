...
pub struct Grid {
	...
	intersection_distances: Vec<i32>,
}
...
impl Grid {
	...
	fn add_location_to_intersections(&mut self, location: Point) {
		let row_distance = (self.starting_point.row as i32 - location.row as i32).abs();
		let col_distance = (self.starting_point.col as i32 - location.col as i32).abs();
		let distance = row_distance + col_distance;

		if distance != 0 {
			// Starting point doesn't count
			self.intersection_distances.push(distance);
		}
	}

	pub fn get_closest_intersection_manhattan(&mut self) -> i32 {
		self.intersection_distances.sort();
		self.intersection_distances[0]
	}
}
